...
//    Level::clearColor = sf::Color(200,200,200);
 ...
 //   if( !init_curveChain( fileName.c_str() ) ) return false;
    ...
    fin >> fileName;
    std::ifstream finFans( fileName.c_str() );
 //   if( SSvec.size() < 3 || !fanA.init( fileName.c_str(), &( SSvec[2] ) ) ) return false;
    if( !finFans ) return false;
    unsigned int numFans; finFans >> numFans;
    fanVec.reserve( numFans );
 //   rotSpeedVec.resize( numFans, 0.0 );// assigned in init multi control
    operCodeVec.reserve( numFans );

    if( SSvec.size() < 3 ) return false;
    char operCode = 'N';
    finFans >> operCode;
    fanVec.push_back( persFan( finFans, &( SSvec[2] ) ) );
    operCodeVec.push_back( operCode );

    finFans >> operCode;
    float Rmin, Rmax, Nrot; finFans >> Rmin >> Rmax >> Nrot;
    ...
    fanVec.push_back( persFan( finFans, pfR, &( SSvec[3] ) ) );
    operCodeVec.push_back( operCode );

    finFans >> operCode;
    finFans >> Af >> Bf;
    pfR = [Af,Bf]( float a ){ return Af*Bf/sqrtf( Af*Af*sinf(a)*sinf(a) + Bf*Bf*cosf(a)*cosf(a) ); };
    fanVec.push_back( persFan( finFans, pfR, nullptr ) );
    operCodeVec.push_back( operCode );
    // 5 point star
    finFans >> operCode >> rotSpeedVec[3];
    finFans >> Bf >> Af;// center to tip
//    Af *= Bf;
//    Af = 0.5528f*Bf;
    pfR = [Af,Bf]( float a )
    {
    ...
    if( SSvec.size() < 4 ) return false;
    fanVec.push_back( persFan( finFans, pfR, &( SSvec[4] ) ) );
    operCodeVec.push_back( operCode );
...
//    ma_xz.pRider = nullptr;
 ...
 //   cluster_1.doZ_order = false;
    cluster_1.showBound = true;
    cluster_1.showCenter = true;
...
void lvl_terrainPerspective::init_Zorder()
{
    ...
    pPtAllVec.reserve( fullSize );
    for( persBox_wf& rPBwf : PBwf_vec ) pPtAllVec.push_back( &rPBwf );
    for( persMat& rPM : persMat_vec ) pPtAllVec.push_back( &rPM );
 //   for( persCurve& rPCv : curveVec ) pPtAllVec.push_back( &rPCv );
 ...
 //   for( persFan& rPF : fanVec ) pPtSortVec.push_back( &rPF );
    pPtSortVec.push_back( &coneBall );
    pPtSortVec.push_back( &parBall );
    pPtSortVec.push_back( &paraChuteBall );
    pPtSortVec.push_back( &paraChuteBall2 );
    pPtSortVec.push_back( &cluster_1 );

    // temp to tshoot a crash. nullptr in vector?
    std::cout << "\nPBwf_vec.size() = " << PBwf_vec.size();
    std::cout << "\nquadAniVec.size() = " << quadAniVec.size();
    std::cout << "\npPtAllVec = " << pPtAllVec.size();
    for( persPt* pPt : pPtAllVec ) if( !pPt ) std::cout << " null";
    std::cout << "\npPtSortVec = " << pPtSortVec.size();
    for( persPt* pPt : pPtSortVec ) if( !pPt ) std::cout << " null";

 //   std::sort( pPtAllVec.begin(), pPtAllVec.end(), persPt::compare );// crash. ok without persBox_wf
...
bool lvl_terrainPerspective::handleEvent( sf::Event& rEvent )
{
    if ( rEvent.type == sf::Event::KeyPressed )
    {
        ...
        else if( rEvent.key.code == sf::Keyboard::Z ) camSpeed = 0.0f;
    ...
    //    else if( rEvent.key.code == sf::Keyboard::P ) P_KeyDown = false;// pitch
     //   else if( rEvent.key.code == sf::Keyboard::R ) R_KeyDown = false;// roll
        ...
        else if( rEvent.key.code == sf::Keyboard::B ) tRoll = 2.0f*vec2f::PI/poiAngVel;// start barrel roll
        ...
        else if( rEvent.key.code == sf::Keyboard::C )// show cluster bounds
        {
            cluster_1.showBound = !cluster_1.showBound;
            cluster_1.showCenter = !cluster_1.showCenter;
        }
        else if( rEvent.key.code == sf::Keyboard::P )// place or return surfer
        {
...
void lvl_terrainPerspective::update_global( float dt )
{
 ...
 //   if( doCoast ) persPt::camPos += hu*(camSpeed*dt);
    if( doCoast ) persPt::camPos += persPt::camDir*(camSpeed*dt);
   ...
   //         if( Y_KeyDown ) rotate_curveChain( 'Y', move_LtRt*rotSpeedChain*dt );
    ...
    if( keepLevel ) persPt::keepXuLevel();// keep xu level
    else if( tRoll > 0.0f )// barrel roll
    {
        if( tRoll > dt )
        {
            persPt::roll( poiAngVel*dt );
            tRoll -= dt;
        }
        else
        {
            persPt::roll( poiAngVel*tRoll );
            tRoll = 0.0f;
        }
    }
...
void lvl_terrainPerspective::update( float dt )
{
    ...
    for( unsigned int n = 0; n < fanVec.size(); ++n )
    {
        ...
        {
            fanVec[n].yaw( dAngle );
            fanVec[n].pitch( dAngle );
            fanVec[n].roll( dAngle );
        }
    //    fanVec[n].update(dt);
    }
...
void lvl_terrainPerspective::draw( sf::RenderTarget& RT ) const
{
 ...
 //   for( const persBox_wf& rPBwf : PBwf_vec ) if( rPBwf.doDraw ) rPBwf.draw(RT);// temp. Should be in pPtSortVec below
...
bool lvl_terrainPerspective::init_MultiControl( multiSelector* pMS, buttonValOnHit* pStrip, std::istream& is )
{
    ...
    camAccel = pSD->xCurr;
    //  move_xyVel = translation speed via arrow keys
    ...
    // surfVX0
    (++pSD)->pSetFunc = [this](float x) { surfVX0 = x; };
    surfVX0 = pSD->xCurr;
    // poiAngVel
    (++pSD)->pSetFunc = [this](float x) { poiAngVel = x; };
    poiAngVel = pSD->xCurr;
    // rotation speeds
...
/*
matAdapter2ndOrder_xz* lvl_terrainPerspective::get_ball_ma_pair()
{
    persBall* pPB = nullptr;
    matAdapter2ndOrder_xz* pMA = nullptr;
    for( persBall& rPB : ballPool )
    {
    ...
    for( matAdapter2ndOrder_xz& rMA : ma_xzPool )
    {
        if( !rMA.pRider )// available
        {
            pMA = &rMA;
            break;
        }
...
/*    for( persFan& rPF : fanVec )
    {
        if( rPF.hit( PB.pos - PB.vel*dt, PB.pos, P, vu ) )
        {
            PB.setPosition( P + PB.Rbound*vu );
            PB.vel = vu*PB.vel.mag();
            return;
    ...
    if( fanA.hit( PB.pos - PB.vel*dt, PB.pos, P, vu ) )
    {
        PB.setPosition( P + PB.Rbound*vu );
        PB.vel = vu*PB.vel.mag();
        return;
    //    PB.vel -= 2.0f*( PB.vel.dot(fanA.Zup) )*fanA.Zup;
    }

    if( fanB.hit( PB.pos - PB.vel*dt, PB.pos, P, vu ) )
    {
        PB.setPosition( P + PB.Rbound*vu );
        PB.vel = vu*PB.vel.mag();
        return;
    ...
    if( fanC.hit( PB.pos - PB.vel*dt, PB.pos, P, vu ) )
    {
        PB.setPosition( P + PB.Rbound*vu );
        PB.vel = vu*PB.vel.mag();
        return;
...
/*
int lvl_terrainPerspective::dblClickPoll( float dt )// call in update(). 0 = no event, 1 = left dbl, -1 = right dbl
{
    static float tElap = 0.0f;
    ...
    // handle click events
    int bce = 0;
    if( button::clickEvent_Lt() == 1 ) bce = 1;
    else if( button::clickEvent_Rt() == 1 ) bce = -1;
    ...
    if( bce != 0 )
    {
        ...
        else if( bce*tElap < 0.0f )// 2nd click contrary
        {
            tElap = 0.0f;// reset
            bce = 0;// not a double
        }
...
int lvl_terrainPerspective::dblClickPoll( float dt )// call in update(). 0 = no event, 1 = left dbl, -1 = right dbl
{
    ...
    if( tElap != 0.0f )
    {
        TE = tElap > 0.0f ? 1 : -1;
        tElap += TE*dt;
        if( TE*tElap > dblClick_tLimit + 0.1f )
        {
            tElap = 0.0f;// time out
            return 0;
        }
    ...
    // handle click events
    int bce = button::clickEvent_Lt() == 1 ? 1 : 0;
    if( bce == 0 && button::clickEvent_Rt() == 1 )
        bce = -1;

    if( bce != 0 )
    {
        // handle contrary input?
        if( bce*tElap < 0.0f )
        {
            tElap = 0.0f;
            return 0;
        ...
        if( tElap == 0.0f ) tElap = bce*dt;// 1st click
        else if( bce*tElap <= dblClick_tLimit )// 2nd click
        {
            tElap = 0.0f;
            return bce;
        }
...
int lvl_terrainPerspective::dblClickPoll( float dt )// call in update(). 0 = no event, 1 = left dbl, -1 = right dbl
{
    ...
    {
        tElap -= dt;
        if( tElap < -1.0f*( dblClick_tLimit + 0.1f ) )
        {
            tElap = 0.0f;
            return 0;
        }
    ...
    if( button::clickEvent_Lt() == 1 )
    {
        ...
        if( tElap == 0.0f ) tElap = dt;
        else if( tElap <= dblClick_tLimit )
        {
            tElap = 0.0f;
            return 1;
        }
