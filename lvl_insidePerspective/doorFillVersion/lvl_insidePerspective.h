...
class lvl_insidePerspective : public Level
{
 ...
 //   sf::Vertex pictQuad[4];// display here - see type persIsland
 //   persDoor* pDoor_pict = nullptr;
  //  persRoom* pRoom_pict = nullptr;
    // use below to update pictQuad positions
 //   vec3f* pCorner[4];// point to: pCorner[j] = doorA.quadPos[k] map depends on doorA.moveDirection (but not moveMode)
 //   void update_pictQuad_tc();
    ...
    // for door use to draw image through open door
    void updateOutFromIn( float dt );
    void drawOutFromIn( sf::RenderTarget& RT );
