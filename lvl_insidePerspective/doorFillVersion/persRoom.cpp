...
bool persRoom::init( std::istream& is, std::vector<spriteSheet>& rSSvec )
{
    ...
    if( numDoors > 0 )
    {
      ...
      //     pDoor->initFill( nullptr, nullptr, [this]( float dt ){ update_inFromOut(dt); }, [this]( sf::RenderTarget& RT ){ draw_inFromOut(RT); } );
       //    pDoor->updateInFromOut = [this]( float dt ){ update_inFromOut(dt); };
        //   pDoor->drawInFromOut = [this]( sf::RenderTarget& RT ){ draw_inFromOut(RT); };
        }
    }
...
void persRoom::update_inFromOut( float dt )
{
 //   vec3f CdirSv = persPt::camDir;
 //   vec3f sep = pos - persPt::camPos;
 //   persPt::camDir = sep/sep.mag();// view direction for these updates
    ...
    for( persQuad& PQ : PQwallVec )// unsorted on walls, floor and ceiling (in and out)
    {
        vec3f sep = PQ.pos - persPt::camPos;
        if( sep.dot( PQ.Nu ) > 0.0f )
            PQ.update(dt);
    }
    // unsorted on walls, floor and ceiling (in and out)
    for( persPt* pPt : pPtSurfaceVec ){ pPt->update(0.0f); }// unsorted on walls, floor and ceiling (inside only)
    // unsorted on walls, floor and ceiling (inside only)
    for( persPt* pPt : pPtVec ) pPt->update(0.0f);// any persPt type - sorted

 //   persPt::camDir = CdirSv;// restore
}

void persRoom::draw_inFromOut( sf::RenderTarget& RT )
{
    RT.draw( ceilingQuad, 4, sf::Quads );
    RT.draw( floorQuad, 4, sf::Quads );

    for( const persWall* pPW : pWallVec )
    {
        vec3f sep = pPW->pos - persPt::camPos;
        if( sep.dot( pPW->Nu ) > 0.0f )
            pPW->draw(RT);
    ...
    for( const persPt* pPt : pPtSurfaceVec ){ pPt->draw(RT); }
