...
bool persRoom::init( std::istream& is, std::vector<spriteSheet>& rSSvec )
{
    ...
    if( numDoors > 0 )
    {
        ...
        for( size_t i = 0; i < numDoors; ++i )
        {
            ...
            Pos += pos;// Pos given relative to this position
            doorVec.push_back( persDoor( rSSvec[SSnum], SetNum, FrIdx, MoveDirection, Pos, Nu, Scale, sf::Color(rd,gn,bu) ) );
            doorVec.back().moveMode = MoveMode;
            doorVec.back().moveSpeed = MoveSpeed;
        }
    }
...
//    std::cerr << "\n**** numInWallpPt = " << numInWallpPt;
    ...
    std::cout << "\nPQwallVec.size() = " << PQwallVec.size() << " chairVec.size() = " << chairVec.size() << " doorVec.size() = " << doorVec.size();
...
{
    if( !doorVec.empty() ) { for( persDoor& rPD : doorVec ) rPD.update(dt); }
}
...
void persRoom::draw( sf::RenderTarget& RT ) const
{
    ...
    else
    {
        ...
        for( const persDoor& PD : doorVec )
        { if( ( persPt::camPos - PD.pos ).dot( PD.Nu ) > 0.0f ) PD.draw(RT); }
    }
