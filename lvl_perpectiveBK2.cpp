...
//    fragShotA.Rdet = 500.0f;
 ...
 //   totalSize += quadBoxVec.size();
    ...
    totalSize += 5;// for messagePQA, shadow4_PQB, fragShotA, droneQuad, coasterCluster
...
bool lvl_perpective::handleEvent( sf::Event& rEvent )
{
 ...
 //   static vec3f camDirSave, xuSave, yuSave;
    ...
    if ( rEvent.type == sf::Event::KeyPressed )
    {
        ...
        else if( rEvent.key.code == sf::Keyboard::B )// trigger superBall position return
        {
            if( !sbReturning )
            {
                sbReturning = true;
     ...
     //       if( dAr*dAr < rollRate*rollRate ) dAr = 0.0f;// assure ability to stop rolling
        ...
        else
        {
            ...
            else
            {
                ...
                if( pArButt && pArButt->sel && U*U < 0.9f )// apply roll with limited pitch
                {
