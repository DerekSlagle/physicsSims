...
class lvl_terrainPerspective : public Level
{
 ...
 //   void updatePane();// now a static persPt method
    ...
    // 2nd newest persPt type
    persFanWide pf_wideA;
    // again
    terrainGrid groundGrid;

    sf::CircleShape theSun, theMoon;
    bool doDrawSun = false, doDrawMoon = false;
    vec3f moonDir;// moon to camera
    void update_sun();
 ...
 //   float get_BezSlope( float u, float u1, float u2 )const;
    ...
    bool init_fans( const char* fileName );
...
/*    std::vector<persCurve> curveVec;
    ...
    void init_Zorder();
    void update_Zorder();
    void analyze_Zorder()const;
    bool analyze_flag = false;// key 'A'
