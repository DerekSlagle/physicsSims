...
const float waveSeg::PIx2 = 6.283185f;
...
void waveSeg::init( std::istream& is )
{
 ...
 //   len = L.mag();// cached! Yeah!
    ...
    float L_wv = 100.0f;
    is >> Amp_wvRt >> L_wv;
    if( L_wv < len/100.0f ) L_wv = len/100.0f;
    K_wvRt = PIx2/L_wv;
...
void waveSeg::initUnderView()
{
    wvVtxVec.resize( 4*Npts_wv );
        ...
        underQuad[1].position.x += len;// up rt
        underQuad[2].position.x += len; underQuad[2].position.y = pos.y + Depth;// dn rt
