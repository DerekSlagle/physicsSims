...
void lineSegFlip::init( vec2d pt1, vec2d pt2, float Angle, float RotVel, sf::Color clr )
{
    lineSeg::init( pt1, pt2, clr );
    rotVelFlip = RotVel;
    Lup = L.Rotate(-Angle);
    Ldn = L;
    rotVel = 0.0f;
...
void lineSegFlip::to_file( std::ofstream& fout )const
{
    fout << "F\n";
    segHit::to_file(fout);
