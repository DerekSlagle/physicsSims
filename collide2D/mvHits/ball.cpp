...
#include "../segHits/arcSeg.h"

// static member function
unsigned int ball::hitAll( std::vector<ball>& ballVec )
{
    unsigned int hitCount = 0;
    for( size_t i = 0; i + 1 < ballVec.size(); ++i )
        for( size_t j = i + 1; j < ballVec.size(); ++j )
        {
           if( ballVec[i].hit( ballVec[j] ) )
            ++hitCount;
        }
...
bool ball::intersect( const regPolygon& py )const { return py.intersect( *static_cast<const ball*>(this) ); }
bool ball::intersect( const mvHit& mh )const { return mh.intersect( *static_cast<const ball*>(this) ); }

bool ball::intersect( const ball& rB )const { return ( pos - rB.pos ).dot( pos - rB.pos ) <= (r + rB.r)*(r + rB.r); }// touching
