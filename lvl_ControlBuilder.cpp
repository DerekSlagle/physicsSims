...
bool lvl_ControlBuilder::init()
{
    ...
    rd = orGate_toCopy.wireToNext[0].color.r;
    std::cout << "\n orGate_toCopy.wireToNext[0].color.r = " << rd;
...
//    addOrGateButt.init( fin, nullptr );
 ...
 //   addBus2outButt.init( fin, nullptr );
    ...
    modeRadioButt.pFuncIdx = [this](size_t mode )
    {
        ...
        pLS_moveCon = pLS_moveSw = nullptr;
//        pOrGateSel = nullptr;
        setRBmode( mode );
    };
    ...
    idFwdLeadButt.pFuncClose = [this]()
    {
        if( pTermSel && pTermFwdLeads.size() < pathDir.size() )
        {
            ...
            pTermSel->tmLabel.setString("fwd");
    ...
    idRevLeadButt.pFuncClose = [this]()
    {
        if( pTermRevLeads.size() < pathDir.size() )
        {
            if( pTermSel )
            {
                ...
                pTermSel->tmLabel.setString("rev");
            }
            else
            {
                pTermRevLeads.clear();
    ...
    if( pathIsComplete )
    {
        if( pTermFwdLeads.size() == pathDir.size() && pTermRevLeads.size() == pathDir.size() )
        {
            ...
            if( didMove )
            {
                ...
                bool LS0state = pLSfirst ? pLSfirst->state : true;
                for( sf_limitSwitch& LS : LimitSwitchVec ) LS.update( loadPos );

                if( pLSfirst && ( !LS0state && pLSfirst->state ) )// from false to true
                {
                    // equate position perp to LS.Nu
                    sf::Vector2f Tu( pLSfirst->Nu.y, -pLSfirst->Nu.x );
                    sf::Vector2f sep = loadPos - pLSfirst->vtx[2].position;
                    float sepDotTu = sep.x*Tu.x + sep.y*Tu.y;
                    loadPos -= sepDotTu*Tu;
                    loadCS.setPosition( loadPos );
                }
            }
...
//        if( terminalVec.size() < termVecCapacity )
    ...
    //    if( orGateVec.size() < conVecCapacity )
        ...
        //    pOG->reBindLambdas();// instead of copy ctor definition
            pOG->setPosition( button::msePos() );
            pTermBindFromOK.push_back( pOG );
        //    pTermBindFromOK.push_back( &pOG->tA );// pNext no longer available ( = this now )
        //    pTermBindFromOK.push_back( &pOG->tB );//
            pTermBindToOK.push_back( &pOG->tA );
            pTermBindToOK.push_back( &pOG->tB );
            std::cout << "\n orGate added";
            typeOrderVec.push_back('F');

            // color copying issue: back to custom copy ctor
            // which relies on default operator =, then corrects internal pointer assigns with reBindLambdas()
            unsigned int rd = orGate_toCopy.wireToNext[0].color.r;// why was this not copied?
            std::cout << "\n orGate_toCopy.wireToNext[0].color.r = " << rd;// getting black as expected
            rd = pOG->wireToNext[0].color.r;// why was this not copied?
            std::cout << "\n pOG->wireToNext[0].color.r = " << rd;// getting white (default color) = total confusion. Color not assigned
            // will forcing it even work?
            pOG->wireToNext[0].color = pOG->wireToNext[1].color = sf::Color::Black;// yes, this works
        }
  ...
  //  }
    ...
    else// 1st click = selection for move
    {
     ...
     /*       if( !hit )// orGate?
            ...
            {           // is a terminal type, but it has 2 separately movable parts so sf_limitSwitch pointers are needed
                sf_terminal* ptHit = nullptr;
                {
...
void lvl_ControlBuilder::saveControls()const
{
    ...
    for( char type : typeOrderVec )
    {
        switch( type )
        {
            ...
            fout << ' ' << orGateVec[ typeCount[4] ].bindIdx;
            ++typeCount[4];
            break;
