...
//    fragShotA.Rdet = 500.0f;
    ...
    wfBoxVec.reserve( numBoxes + 27 );
    std::cerr << "\n numBoxes = " << numBoxes;

    for( size_t i = 0; i < numBoxes; ++i ) wfBoxVec.push_back( persBox_wf(fin) );
    // a ring of 12 pb_wf
    ...
    for( int i = 0; i < 24; ++i )
    {
        ...
        wfBoxVec.push_back( persBox_wf(Pos, Sz, sf::Color::Red) );
        persBox_wf& rpbwf = wfBoxVec.back();
 ...
 //   Pos.y = 200000.0f;
    fin >> Pos.y;
    Pos.z = persPt::Z0;
    wfBoxVec.push_back( persBox_wf( Pos, 10.0f*Sz, sf::Color::White ) );
    // south pole
    Pos.y *= -1.0f;
    wfBoxVec.push_back( persBox_wf( Pos, 10.0f*Sz, sf::Color::White ) );
 ...
 //   Sz.x = Sz.y = Sz.z = 8.0f*harmOsciBall.Rbound;
    wfBoxVec.push_back( persBox_wf( HOpos, Sz, sf::Color::White ) );
 ...
 //   { Pos.x = A*cosf(par); Pos.y = B*sinf(par); Pos.z = Zmax*cosf( fN*par ); };
  //  curve1.init( fin, p_InitFunc );
    ...
    // the coasterCluster
    coasterCluster.pPersVec.reserve( coasterBallVec.size() + coasterQuadVec.size() );
    for( persBall& PB : coasterBallVec ) coasterCluster.pPersVec.push_back( &PB );
    for( persQuad& PQ : coasterQuadVec ) coasterCluster.pPersVec.push_back( &PQ );
    if( !coasterCluster.init() ) { std::cout << "\n cluster init fail!!"; return false; }
...
{
    // pointers for mainIslandCluster
    for( persQuad& PQ : terrainPQvec ) mainIslandCluster.pPersVec.push_back( &PQ );
    for( persQuad& PQ : quadVec ) mainIslandCluster.pPersVec.push_back( &PQ );
    for( shadowAdapter& rSA : shAdaptVec ) mainIslandCluster.pPersVec.push_back( &rSA );
    for( persShadow& PS : shadowVec ) mainIslandCluster.pPersVec.push_back( &PS );
 //   mainIslandCluster.pPersVec.push_back( &harmOsciBall );
    mainIslandCluster.pPersVec.push_back( &shadow4_PQB );

    // the rest
    size_t totalSize = wfBoxVec.size() + quadAniVec.size() + ballVec.size();
 //   totalSize += quadVec.size();// shadowAdapters update and draw these now
 //   totalSize += quadBoxVec.size();// shadowAdapters update and draw these now
 //   totalSize += shAdaptVec.size();
 //   totalSize += terrainPQvec.size();
    totalSize += CarVec.size();
    totalSize += pIslandVec.size();
    totalSize += hyperVec_1.size() + hyperVec_2.size();
    totalSize += freeBallVec.size();
//    totalSize += shadowVec.size();
    totalSize +=2;// for messagePQA, fragShotA
    totalSize += 3;// for droneQuad, coasterCluster, mainIslandCluster
    totalSize += 4;// for superBall, calcBall, multiBallBox, Pend
    p_persPtVec.reserve( totalSize );
    for( persBox_wf& box : wfBoxVec ) p_persPtVec.push_back( &box );
...
//    p_persPtVec.push_back( &shadow4_PQB );
    ...
    p_persPtVec.push_back( &coasterCluster );
    p_persPtVec.push_back( &mainIslandCluster );
    ...
    // after filling pPersVec and pTargetVec
    mainIslandCluster.init();
}
...
bool lvl_perpective::handleEvent( sf::Event& rEvent )
{
 ...
 //   static vec3f camDirSave, xuSave, yuSave;
    ...
    else if ( rEvent.type == sf::Event::KeyReleased )
    {
        ...
        else if( rEvent.key.code == sf::Keyboard::P )// place/ release camera on coaster
        {
...
//        to_SF_string( pitchNumMsg, U );
    ...
    //    if( pPaneButt && pPaneButt->sel ) updatePane();
        ...
        if( pCoasterPath )
        {
            coasterCluster.update(dt);
            if( coasterCluster.doDraw )
  ...
  //      persLS.update(dt);
        for( persBox_wf& box : wfBoxVec ) box.update(dt);
    //    for( persBox_quad& Qbox : quadBoxVec ) Qbox.update(dt);// now handled by shadowAdapters
    //    for( persQuad& PQ : quadVec ) PQ.update(dt);// 1 target only! mainIslandCluster

    //    for( persQuad& PQ : terrainPQvec ) PQ.update(dt);
        mainIslandCluster.update(dt);

    //    for( persShadow& PS : shadowVec ) PS.update(dt);// mainIslandCluster
    //    for( shadowAdapter& rSA : shAdaptVec ) rSA.update(dt);// mainIslandCluster
    ...
    //    curve1.update(dt);
        handleHyperJump(dt);
    //    shadow4_PQB.update(dt);// mainIslandCluster
...
void lvl_perpective::update_Zorder()
{
    ...
    persCluster &CC = coasterCluster, &MC = mainIslandCluster;
    CC.numToDraw = MC.numToDraw = 0;

    if( anyShots_inUse )
        for( persBall& PB : ballVec )
            if( PB.inUse && PB.doDraw )
            {
                if( CC.inBounds( PB.pos ) )
                {
                    if( CC.numToDraw < CC.pSortedVec.size() ) CC.pSortedVec[ CC.numToDraw ] = &PB;
                    else CC.pSortedVec.push_back( &PB );
                    ++CC.numToDraw;
                }
                else if( MC.inBounds( PB.pos ) )
                {
                    if( MC.numToDraw < MC.pSortedVec.size() ) MC.pSortedVec[ MC.numToDraw ] = &PB;
                    else MC.pSortedVec.push_back( &PB );
                    ++MC.numToDraw;
                }
                else
                {
    ...
    if( anyAnis_inUse )
        for( persQuadAni& PQA : quadAniVec )
            ...
            {
                if( CC.inBounds( PQA.pos ) )
                {
                    if( CC.numToDraw < CC.pSortedVec.size() ) CC.pSortedVec[ CC.numToDraw ] = &PQA;
                    else CC.pSortedVec.push_back( &PQA );
                    ++CC.numToDraw;
                }
                else if( MC.inBounds( PQA.pos ) )
                {
                    if( MC.numToDraw < MC.pSortedVec.size() ) MC.pSortedVec[ MC.numToDraw ] = &PQA;
                    else MC.pSortedVec.push_back( &PQA );
                    ++MC.numToDraw;
                }
                else
                {
    ...
    {
         if( CC.inBounds( fragShotA.pos ) )
        {
            if( CC.numToDraw < CC.pSortedVec.size() ) CC.pSortedVec[ CC.numToDraw ] = &fragShotA;
            else CC.pSortedVec.push_back( &fragShotA );
            ++CC.numToDraw;
        }
        else if( MC.inBounds( fragShotA.pos ) )
        {
            if( MC.numToDraw < MC.pSortedVec.size() ) MC.pSortedVec[ MC.numToDraw ] = &fragShotA;
            else MC.pSortedVec.push_back( &fragShotA );
            ++MC.numToDraw;
        }
        else
        {
            if( numToDraw >= sz ){ std::cerr << " sz exceeded "; }
            else pPtVec_4list[ numToDraw++ ] = &fragShotA;
        }
    }

    if( harmOsciBall.inUse && harmOsciBall.doDraw )
    {
         if( CC.inBounds( harmOsciBall.pos ) )
        {
            if( CC.numToDraw < CC.pSortedVec.size() ) CC.pSortedVec[ CC.numToDraw ] = &harmOsciBall;
            else CC.pSortedVec.push_back( &harmOsciBall );
            ++CC.numToDraw;
        }
        else if( MC.inBounds( harmOsciBall.pos ) )
        {
            if( MC.numToDraw < MC.pSortedVec.size() ) MC.pSortedVec[ MC.numToDraw ] = &harmOsciBall;
            else MC.pSortedVec.push_back( &harmOsciBall );
            ++MC.numToDraw;
        }
        else
        {
            if( numToDraw >= sz ){ std::cerr << " sz exceeded "; }
            pPtVec_4list[ numToDraw++ ] = &harmOsciBall;
        }
    }
...
void lvl_perpective::hitAll( persBall& PB, float dt )
{
    ...
    // targets within main cluster
    if( mainIslandCluster.inBounds( PB.pos ) )
    {
...
void lvl_perpective::draw( sf::RenderTarget& RT ) const
{
    ...
    if( pCoasterPath && coasterCluster.doDraw )
