...
//    Af = 0.5528f*Bf;
 ...
 //   float yawAngle, pitchAngle; finFans >> yawAngle >> pitchAngle;
 ...
 //   pitchAngle *= vec2f::PI/180.0f;
    ...
    // add shadows
  //  lightDir = cosf( pitchAngle )*( persPt::xHat*sinf( yawAngle ) + persPt::zHat*cosf( yawAngle ) ) - persPt::yHat*sinf( pitchAngle );
 ...
 //   fanVec[caIdx].initAxes( sf::Color::Yellow, fanVec[2].Rbound*1.5f, 20.0f, 6.0f, &label );
...
//    ma_xz.pRider = nullptr;
   ...
   //     ma_xzPool.push_back( ma_xz );
    ...
    if( qSz > 2 )
    {
        carYuTotal.x = carYuTotal.y = carYuTotal.z = 0.0f;
        // initial fill
        for( unsigned int j = 0; j < qSz; ++j )
        {
            carYuTotal += persPt::yHat;
            carYuQueue.push( persPt::yHat );
        }
    }
...
void lvl_terrainPerspective::init_Zorder()
{
 ...
 //   for( persBox& rPB : persBox_vec ) pPtStillVec.push_back( &rPB );
    ...
    size_t movingSize = persBox_vec.size();// + 5;
...
//    pPtMovingVec.push_back( &coneBall );
 ...
 //   fullSize += quadAniVec.size() + ballPool.size() + shotPool.size();// + fanVec.size();
    ...
    for( persBallTimed& rPBT : shotPool ) pPtSortVec.push_back( &rPBT );
    pPtMovingVec.push_back( &coneBall );
    pPtMovingVec.push_back( &parBall );
    pPtMovingVec.push_back( &paraChuteBall );
    pPtMovingVec.push_back( &paraChuteBall2 );
...
void lvl_terrainPerspective::init_carAnis()
{
    ...
    temp.fUpdateX = [this]( float dX )
    {
        ...
        pCar->Yu = getYuAvg( pCar->Yu );
        pCar->Yu /= pCar->Yu.mag();
        pCar->Xu = pCar->Zu.cross( pCar->Yu );
        pCar->Xu /= pCar->Xu.mag();
        pCar->Yu = pCar->Xu.cross( pCar->Zu );
    ...
    temp.fUpdateX = [this]( float dX )
    {
     //   rotateBasisLH( persPt::yHat, carSpeed*dX/carTurnR, pCar->Xu, pCar->Yu, pCar->Zu );
        vec3f::bank( -gravity.y, carSpeed, -carSpeed/carTurnR, dX, pCar->Xu, pCar->Yu, pCar->Zu );
...
vec3f lvl_terrainPerspective::getYuAvg( vec3f Yu_curr )
{
    carYuTotal -= carYuQueue.front();// this total
    carYuQueue.pop();
    carYuQueue.push( Yu_curr );
    carYuTotal += Yu_curr;//       should remain balanced

    return carYuTotal*( 1.0f/carYuQueue.size() );
}
...
void lvl_terrainPerspective::update_global( float dt )
{
 ...
 //   if( animatedTransition<float>::numAnimating > 0 )
    if( animatedTransition<float>::numAnimating > 1 )
    {
...
void lvl_terrainPerspective::update_Zorder()
{
 ...
 //   for( persPt* pPt2 : pPtMovingVec )
 //       if( pPt2->inUse && pPt2->doDraw && numToDraw < pPtSortVec.size() ) pPtSortVec[ numToDraw++ ] = pPt2;

    for( persBox& PBX : persBox_vec )
        if( PBX.inUse && PBX.doDraw && numToDraw < pPtSortVec.size() ) pPtSortVec[ numToDraw++ ] = &PBX;
...
void lvl_terrainPerspective::hitAll( persBall& PB, float dt )
{
  ...
  //  static int hitCnt = 0;
    ...
    for( persBox& rPB : persBox_vec )
        if( rPB.persPt::hit( PB.pos - PB.vel*dt, PB.pos, P, PB.vel ) )
        {
         //   PB.setPosition( P );
